...
boot().catch(err => {
  console.error(err);
  $("#boot").innerHTML = `<div class="auth-card"><h2>Couldn't start the CRM</h2><p class="muted">${esc(err.message)}</p><p class="hint">Check the Supabase URL and key in js/config.js.</p></div>`;
});
...
async function enter(user) {
  ...
  if (!LIVE) {
    ...
    b.innerHTML = `<strong>Demo mode.</strong> Sample data is stored only in this browser. To go live, add your Supabase keys to <span class="mono">js/config.js</span> (see README).`;
  }
  renderSide();
  onChange(renderSide);
